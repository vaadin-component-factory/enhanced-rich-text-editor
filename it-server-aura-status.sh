#!/bin/bash
# Check V25 Aura IT server status
# Usage: bash it-server-aura-status.sh [options] [port]
...
if [ "$WAIT_MODE" = true ]; then
    echo "Waiting for Aura IT server on port $PORT (timeout: ${TIMEOUT}s)..."
    for i in $(seq 1 $TIMEOUT); do
        STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:${PORT}/" 2>/dev/null)
        if [ "$STATUS" = "200" ]; then
            echo "Aura IT server ready (HTTP 200)"
            exit 0
        fi
        sleep 1
    done
    echo "Timeout: Aura IT server not ready after ${TIMEOUT}s"
    exit 1
fi

# Normal status check
echo "=== ERTE V25 Aura IT Server Status ==="
