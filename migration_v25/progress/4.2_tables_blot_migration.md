...
- **Sanitizer integration** (whitelist table elements/attributes)
  ...
  - Core ERTE analysis: Static whitelists (like in Core ERTE) do NOT work
  - **Required:** Template-based validation (validate styles/classes against template definition)
...
- Templates define **user-defined CSS classes and styles** on cells
- Core ERTE sanitizer uses **static whitelists** → does NOT work for Tables
...
```
**Pro:** Sicher, präzise
**Con:** Template objects must be available during sanitization
...
- **Core ERTE sanitizer analysis** — Learnings about static vs. template-based validation
