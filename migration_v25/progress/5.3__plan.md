...
1. **No internal scripts** — Replace `v25-build.sh`, `v25-server-start.sh` etc. with plain `mvn` / `mvnw` commands. Internal scripts are dev convenience, not user-facing.
2. **No migration artifact names** — Replace `enhanced-rich-text-editor-v25` with `enhanced-rich-text-editor`, `enhanced-rich-text-editor-tables-v25` with `enhanced-rich-text-editor-tables`. The `-v25` suffix is temporary.
...
- Cross-check method signatures against actual Java source in `enhanced-rich-text-editor-v25/src/`
