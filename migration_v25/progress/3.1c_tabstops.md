...
**Test results (Variant A, after all fixes):** 108 pass, 17 skipped, 0 fail.

**Bug 3: ArrowUp/ArrowDown broken (NEW — found during manual testing):**
Cursor can no longer navigate between lines with ArrowUp/ArrowDown. It gets stuck,
jumping back and forth instead of moving to the next/previous line. Affects both
lines with text and tab-only lines. Worked before the guard wrapping changes.
Likely caused by guard node wrapping interfering with Quill's vertical navigation
logic (which may expect guards as direct children of the embed domNode).
No existing test covers this — needs a new regression test.
**Status:** Not yet investigated. Will address after Variant A/B evaluation.
