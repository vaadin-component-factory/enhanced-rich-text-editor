// Quill 2 / Parchment 3 version of TableHistory
// Changes from V1:
// 1. Parchment.find() -> Quill.find()
// 2. Parchment.create(domNode) -> Quill.find(domNode) or scroll.create() where needed
...
class TableHistory {
  ...
  static insert(change) {
    ...
    if (parentNode) {
      // Change 1+2: Parchment.find() -> Quill.find()
      const _parentNode = Quill.find(parentNode);
      if (_parentNode) {
        // For history re-insertion, the node is detached. In Parchment 3, Registry.create()
        // no longer accepts DOM nodes directly. Use scroll.create() from the parent blot,
        // which wraps an existing DOM node into a blot.
        const _node = _parentNode.scroll.create(change.node);
        if (change.nextNode) {
          const _nextNode = Quill.find(change.nextNode);
          if (_nextNode) {
