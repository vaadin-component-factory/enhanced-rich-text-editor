...
public class EnhancedRichTextEditorTables {
    ...
    protected EnhancedRichTextEditorTables(EnhancedRichTextEditor rte, TablesI18n i18n) {
        ...
        element.addEventListener("table-selected", event -> {
                    ObjectNode eventData = (ObjectNode) event.getEventData();
                    fireEvent(new TableSelectedEvent(
                            this,
                            true,
                            eventData.get("event.detail.selected").asBoolean(),
                            eventData.get("event.detail.cellSelectionActive").asBoolean(),
                            eventData.has("event.detail.template") ? eventData.get("event.detail.template").asText() : null
                    ));
                })
    ...
    private Integer toInteger(ObjectNode object, String key) {
        return object.has(key) && !object.get(key).isNull() ? Double.valueOf(object.get(key).asDouble()).intValue() : null;
    }
    ...
    protected void initToolbarTable() {
        ...
        modifyTableSelectPopup.addSeparator();
        modifyTableSelectPopup.addItem(
     ...
     */
    public void setTemplateIdForCurrentTable(String templateId) {
        if(templatesDialog != null) {
     ...
     */
    private void internalUpdateTemplateForCurrentTable(String template, boolean fromClient) {
     ...
     */
    public void setTableHoverColor(String hoverColor) {
     ...
     * @param hoverColor css color
     */
    public void setTableCellHoverColor(String hoverColor) {
     ...
     */
    public void setTableCellFocusColor(String focusColor) {
     ...
     * @param focusColor css color
     */
    public void setTableFocusColor(String focusColor) {
